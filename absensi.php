...
$sql = "SELECT a.*, s.nama_shift, s.jam_mulai as shift_mulai, s.jam_selesai as shift_selesai FROM absensi a LEFT JOIN shifts s ON a.shift_id = s.id WHERE a.user_id=? AND a.tanggal=?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if ($action == 'checkin') {
        ...
        // Validasi waktu shift (opsional)
        $shift_check = $conn->prepare("SELECT * FROM shifts WHERE id = ? AND is_active = 1");
        $shift_check->bind_param("i", $shift_id);
        $shift_check->execute();
        $shift_data = $shift_check->get_result()->fetch_assoc();
        ...
        if ($insert_stmt->execute()) {
            echo json_encode([
                'success' => true, 
                'message' => "Check-in berhasil pada jam $jam", 
                'jam_masuk' => $jam,
                'shift_nama' => $shift_data['nama_shift']
            ]);
        } else {
        ...
        // Hitung keterlambatan (opsional)
        $keterlambatan = hitungKeterlambatan($jam_masuk, $data['shift_id']);
        
        // Update check-out
        $update_sql = "UPDATE absensi SET jam_keluar=?, durasi=?, keterlambatan=?, status_kehadiran='Hadir' WHERE id=?";
        $update_stmt = $conn->prepare($update_sql);
        if ($update_stmt === false) {
            echo json_encode(['success' => false, 'message' => 'Database error: ' . $conn->error]);
            exit;
        }
        $update_stmt->bind_param("sssi", $jam, $durasi, $keterlambatan, $data['id']);
        
        if ($update_stmt->execute()) {
            echo json_encode([
                'success' => true, 
                'message' => "Check-out berhasil pada jam $jam", 
                'jam_keluar' => $jam, 
                'durasi' => $durasi,
...
function hitungKeterlambatan($jam_masuk, $shift_id) {
    global $conn;
    
    // Ambil jam mulai shift
    $shift_sql = "SELECT jam_mulai FROM shifts WHERE id = ?";
    ...
    $jam_mulai_shift = $shift_data['jam_mulai'];
    
    // Hitung selisih waktu
    $selisih = strtotime($jam_masuk) - strtotime($jam_mulai_shift);
    
    if ($selisih > 0) {
        return gmdate('H:i:s', $selisih);
    } else {
        return '00:00:00';
    }
}
...
$history_sql = "SELECT a.*, s.nama_shift FROM absensi a LEFT JOIN shifts s ON a.shift_id = s.id WHERE a.user_id=? ORDER BY a.tanggal DESC LIMIT 7";
                            ...
                            <div class="status-card bg-success text-white">
                                <div class="row align-items-center">
                                    <div class="col-md-8">
                                        ...
                                        <p class="mb-1">Jam Masuk: <?= $absensi_hari_ini['jam_masuk'] ?></p>
                                        <p class="mb-0">Shift: <?= isset($absensi_hari_ini['nama_shift']) ? $absensi_hari_ini['nama_shift'] : '-' ?></p>
                                    </div>
    ...
    <script>
        ...
        function doCheckout() {
            if (confirm('Apakah Anda yakin ingin check-out?')) {
                ...
                .then(data => {
                    if (data.success) {
                        alert(data.message + (data.durasi ? ' | Durasi: ' + data.durasi : ''));
                        location.reload();
                    } else {
                        alert(data.message);
                    }
