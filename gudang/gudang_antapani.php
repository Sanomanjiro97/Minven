...
$gudang_id = 13;
$gudang_nama = '';
// Ensure database connection exists before preparing statement
if (isset($conn) && $conn instanceof mysqli) {
    $stmt_gudang = $conn->prepare("SELECT nama_gudang FROM gudang WHERE id = ?");
} else {
    $stmt_gudang = false;
    error_log("Database connection not available when trying to fetch gudang data");
}
if ($stmt_gudang) {
...
if (!checkAccess('gudang_antapani', 'view')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reset_stok'])) {
    if (!checkAccess('gudang_antapani', 'delete')) {
    ...
    if (!isset($conn) || !($conn instanceof mysqli)) {
        throw new Exception("Database connection not available for stock reset");
    }
    $stmt = $conn->prepare("UPDATE $nama_tabel SET jumlah = 0, stok_awal = 0, stok_terpakai = 0, stok_sisa = 0 WHERE gudang_id = ?");
...
$has_barcode_dus = false;
if (isset($conn) && $conn instanceof mysqli && function_exists('db_has_column')) {
    $has_barcode_dus = db_has_column($conn, 'barang', 'barcode_dus');
}
$has_konversi_table = false;
$tblRes = false;
if (isset($conn) && $conn instanceof mysqli) {
    $tblRes = $conn->query("SHOW TABLES LIKE 'konversi_satuan_barang'");
} else {
    error_log("Database connection not available when checking for konversi_satuan_barang table");
}
if ($tblRes instanceof mysqli_result) {
    $has_konversi_table = $tblRes->num_rows > 0;
    $tblRes->free();
}

$select_barcode_dus = $has_barcode_dus ? "b.barcode_dus" : "NULL AS barcode_dus";
$select_konversi_dus = "NULL AS konversi_dus";
if ($has_konversi_table) {
    $select_konversi_dus = "COALESCE(
        (
            SELECT kb.nilai_konversi
            FROM konversi_satuan_barang kb
            INNER JOIN satuan s_asal ON kb.satuan_asal_id = s_asal.id
            WHERE kb.barang_id = gs.barang_id
                AND LOWER(s_asal.nama_satuan) LIKE '%dus%'
                AND kb.satuan_tujuan_id = b.satuan_id
            ORDER BY kb.updated_at DESC, kb.id DESC
            LIMIT 1
        ),
        (
            SELECT CASE WHEN kb.nilai_konversi > 0 THEN (1 / kb.nilai_konversi) ELSE NULL END
            FROM konversi_satuan_barang kb
            INNER JOIN satuan s_tujuan ON kb.satuan_tujuan_id = s_tujuan.id
            WHERE kb.barang_id = gs.barang_id
                AND LOWER(s_tujuan.nama_satuan) LIKE '%dus%'
                AND kb.satuan_asal_id = b.satuan_id
            ORDER BY kb.updated_at DESC, kb.id DESC
            LIMIT 1
        )
    ) AS konversi_dus";
...
// Check if database connection exists before calling the reset function
if (isset($conn) && $conn instanceof mysqli) {
    resetStokHarian($conn);
} else {
    error_log("Cannot execute daily stock reset: Database connection not available");
}
$sql = "SELECT
    gs.id,
    gs.barang_id,
...
GROUP BY gs.id";
// Safely execute query only if database connection is valid
$result = (isset($conn) && $conn instanceof mysqli) ? $conn->query($sql) : false;

// Debug information
if (!$result) {
    $error_msg = isset($conn) && $conn instanceof mysqli ? $conn->error : "Database connection not available";
    error_log("Database error in gudang_antapani.php: " . $error_msg);
    $_SESSION['error'] = "Error executing query: " . $error_msg;
} else {
            ...
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2">
                        <?php $canAdd = checkAccess('gudang_antapani', 'add'); $canDelete = checkAccess('gudang_antapani', 'delete'); ?>
                        <?php if ($canAdd): ?>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-lg-3 col-md-6">
                        <select class="form-select" id="filter_lokasi" name="filter_lokasi">
                            ...
                            $lokasi_result = (isset($conn) && $conn instanceof mysqli) ? $conn->query($lokasi_query) : false;
                            if ($lokasi_result) {
                    ...
                    <table class="table table-hover mb-0" id="inventoryTable">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 120px;">Barcode Konversi</th>
                        ...
                        <tbody>
                            ...
                            if ($result && $result->num_rows > 0) {
                                ...
                                $stokMinimum = 0;
                                $stokHabis = 0;
                                $canEditGlobal = checkAccess('gudang_antapani', 'edit');
                                $canDeleteGlobal = checkAccess('gudang_antapani', 'delete');
                            ...
                            ?>
                            <tr class="<?= $stok_class ?>"
                                data-barcode="<?= htmlspecialchars(trim((string)($row['barcode'] ?? ''))) ?>"
                                data-barcode-dus="<?= htmlspecialchars(trim((string)($row['barcode_dus'] ?? ''))) ?>"
                                data-barcode-konversi="<?= htmlspecialchars(trim((string)($row['barcode_dus'] ?? ''))) ?>"
                                data-konversi-dus="<?= htmlspecialchars(trim((string)($row['konversi_dus'] ?? ''))) ?>"
                                data-barang-id="<?= (int)($row['barang_id'] ?? 0) ?>">
                                ...
                                <td>
                                    <span class="text-muted"><?= htmlspecialchars($row['barcode_dus'] ?? '-') ?></span>
                                </td>
                                <td>
                            ...
                            <?php endwhile;
                                ...
                                if (!$result) {
                                    $error_msg = isset($conn) ? $conn->error : 'Database connection error: $conn variable is not defined';
                                    echo "<tr><td colspan='17' class='text-center text-danger'>Error executing query: " . htmlspecialchars($error_msg) . "</td></tr>";
                                } else {
                                    echo "<tr><td colspan='17' class='text-center text-muted'>Tidak ada data stok untuk gudang ini.</td></tr>";
                                }
                            }
                            ?>
                        </tbody>
    ...
    <div class="modal fade" id="addStokModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form id="addStokForm" action="process_antapani.php" method="POST">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select select2" id="barang_id" name="barang_id" required>
                                ...
                                $barang_result = (isset($conn) && $conn instanceof mysqli) ? $conn->query($barang_query) : false;
                                if ($barang_result) {
    ...
    <div class="modal fade" id="stokMasukModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <form id="stokMasukModalForm" action="process_antapani.php" method="POST">
                    <div class="modal-body">
    ...
    <div class="modal fade" id="editStokModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="editStokForm" action="process_antapani.php?action=edit" method="POST">
                    <div class="modal-body">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function applyAllFilters() {
        ...
        rows.forEach(row => {
                    ...
                    const barcodeDus = (row.cells[3]?.textContent || '').toLowerCase();
                    const namaBarang = (row.cells[4]?.textContent || '').toLowerCase();
                    const kategori = (row.cells[5]?.textContent || '').toLowerCase();
                    const location = (row.cells[7]?.textContent || '').trim();
                    const status = (row.cells[11]?.textContent || '').trim();

                    const matchSearch = !searchTerm || kodeBarang.includes(searchTerm) || barcode.includes(searchTerm) || barcodeDus.includes(searchTerm) || namaBarang.includes(searchTerm) || kategori.includes(searchTerm);
    ...
    window.updateStok = function(input) {
        ...
        const prevFinalAttr = input.getAttribute('data-prev-final');
        const prevFinal = prevFinalAttr != null ? parseFloat(prevFinalAttr) : NaN;
        const lastGoodFinal = !isNaN(prevFinal) ? prevFinal : (isNaN(stokAwal) ? 0 : stokAwal);

        if (isNaN(sisaStok) || sisaStok < 0 || sisaStok > stokAwal) {
            showToast('Nilai stok tidak valid', 'error');
            input.value = lastGoodFinal;
            return;
        }
        ...
        input.setAttribute('data-syncing', '1');
        ...
        .then(data => {
            if(data.success) {
                ...
                const stokMin = parseFloat(row.cells[12]?.textContent || '0');
                ...
                if (row.cells[10]) row.cells[10].textContent = number_format(stokTerpakai);
        ...
        .finally(() => {
            ...
            input.removeAttribute('data-syncing');
    ...
    window.deleteStok = function(id) {
        if (confirm('Apakah Anda yakin ingin menghapus stok ini?')) {
            ...
            form.method = 'POST';
            form.action = 'process_antapani.php';
...
<script src="includes/barcode_scan_stok_final.js?v=<?= urlencode((string)@filemtime(__DIR__ . '/includes/barcode_scan_stok_final.js')) ?>"></script>
</body>
</html>
