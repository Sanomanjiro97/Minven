...
if (!checkAccess('adjustment_out', 'add')) {
    $_SESSION['error'] = 'Anda tidak memiliki akses untuk menambah adjustment out!';
...
<head>
    ...
    <title>Tambah Adjustment Out - Sistem Inventory</title>
...
<body>
                ...
                <div class="card">
                    <div class="card-header" style="background-color: #dc3545; color: white;">
                        <div class="d-flex justify-content-between align-items-center">
                            <h5 class="card-title"><i class="bx bx-minus-circle me-2"></i>Form Tambah Adjustment Out</h5>
                            <a href="index.php" class="btn btn-secondary">Batal</a>
                        </div>
                    </div>
                    <div class="card-body">
                        <form id="adjustmentOutForm" action="process.php" method="POST">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="keterangan" name="keterangan" rows="3" placeholder="Contoh: Adjustment stok barang expired, Koreksi stok barang rusak"></textarea>
                            </div>

                            <hr>

                            <h5>Detail Barang Adjustment Out</h5>
                            ...
                            <div class="row g-3 mb-3 align-items-end">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="text" class="form-control" id="input_keterangan_item" placeholder="Contoh: Barang expired">
                                </div>
                                <div class="col-md-2">
                                    <button type="button" class="btn btn-danger w-100" id="addItemBtn">Tambah Item</button>
                                </div>
                            </div>
                            ...
                            <button type="submit" class="btn btn-danger" id="submitFormBtn" name="submitFormBtn">Simpan Adjustment Out</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const adjustmentOutForm = document.getElementById('adjustmentOutForm');
            ...
            addItemBtn.addEventListener('click', function() {
                ...
                const stokTersedia = parseInt(selectedOption.getAttribute('data-stok-tersedia')) || 0;
                const totalExisting = items.filter(item => item.barang_id == selectedOption.value).reduce((sum, item) => sum + item.jumlah, 0);
                
                if ((totalExisting + jumlah) > stokTersedia) {
                ...
                if (existingIndex !== -1) {
                    const newTotal = items[existingIndex].jumlah + jumlah;
                    if (newTotal > stokTersedia) {
                        alert('Jumlah melebihi stok tersedia. Stok tersedia: ' + stokTersedia);
                        return;
                    }
                    items[existingIndex].jumlah = newTotal;
                } else {
                    items.push({
            ...
            adjustmentOutForm.addEventListener('submit', function(e) {
                if (items.length === 0) {
                    e.preventDefault();
